...
GitHub repository created 2021-03-26, last updated 2021-07-03

# Summary

WinSetView provides an easy way to globally set Windows Explorer folder views. For example, if you want Details view with a particular selection of column headings enabled across all folders, then WinSetView will do that for you.

WinSetView is comprised of two main files: **WinSetView.hta** (HTML GUI with VBScript code) and **WinSetView.ps1** (PowerShell command line script) and numerous supporting files (see the Files section below for details).

Double-click **WinSetView.hta** to open the GUI. When you click **Submit**, the hta/vbs script will pass your choices as an INI file to WinSetView.ps1, which will make the changes and then restart Explorer.
...
Load settings (i.e. interface selections) from a previously saved INI file.

## Save Settings

Save current settings (i.e. interface selections) to an INI file.
...
This option retains any folder views that have been saved using Explorer's **Apply to Folders** button. With the original WinSetView v1, this was the only method available to keep some non-generic folder settings in addition to the Global settings. This option exists for anyone wishing to use WinSetView v2 in exactly the same manner as WinSetView v1.

With WinSetView v2, there are individual settings for each folder type, making this setting unnecessary. For most users, it will be simpler to set all folder views in *WinSetView* and leave this option unchecked.

If you want to use this option, be sure to use the **Apply to Folders** button in Explorer for each folder type view you want to set. That is, in *Explorer*, set your desired view for **Downloads**, then go to **View**, **Options**, **Change folder and search options**, **View** tab, and click **Apply to Folders**. Repeat those steps for **Documents**, **Music**, **Pictures**, and **Videos**. All of those views will then take precedence over any Global view set with *WinSetView*.
...
![image](https://user-images.githubusercontent.com/79026235/124369686-902dc800-dc3c-11eb-9f79-5d1a59525e1c.png)
...
By default, Explorer has automatic folder type discovery *enabled*. This means Explorer will automatically change the view of a folder based on its contents. The "FolderType"="Generic" registry entry, described above, also disables automatic folder discovery. This setting is applied by WinSetView when you select the **Make All Folders Generic** option.
...
WinSetView is designed to work with any left to right language. There are currently 31 languages included with WinSetView v2. If your language is not included, please contact me.
...
Wherever possible, I have copied over any hand-translated text available from WinSetView v1. I have also done my best to match the folder type names to the terms Windows uses. However, there are many strings that could use a native speaker to make the translations better. 
